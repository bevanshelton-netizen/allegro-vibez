  ...
  return <>
    <header className="site-header">
      ...
      <div className="account-links">
        {session ? <><Link to="/dashboard">Dashboard</Link><button className="text-button" type="button" onClick={signOut}>Log out</button></> : <><Link to="/login">Log in</Link><Link className="pill" to="/register">Join</Link></>}
      </div>
    </header>
    {children}
...
function Dashboard({ session }) {
  ...
  return session ? <Page title="Dashboard"><p>Welcome back, {session.user.email}.</p><div className="cards"><article><h3>Release pipeline</h3><strong>{count===null?'—':count}</strong><p>Total releases in your catalogue.</p></article><article><h3>Catalogue</h3><strong>My Music</strong><p>Track release status and metadata.</p><Link to="/my-music">Open catalogue</Link></article><article><h3>Prosperity</h3><strong>Foundation live</strong><p>Royalty and payout reporting can now build on verified releases.</p></article></div></Page> : <RequireLogin/>
}
