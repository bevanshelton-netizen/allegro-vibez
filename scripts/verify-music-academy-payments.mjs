...
req("amount: 9900" in lib,'Academy server price must be R99')
req("days: 30" in lib,'Academy pass must be 30 days')
req("process.env.IKHOKHA_APP_ID" in lib && "process.env.IKHOKHA_APP_SECRET" in lib,'iKhokha credentials must stay in server environment')
req("createHmac('sha256'" in lib,'HMAC-SHA256 signing is required')
